...
const sharedContent = `// Shared Utilities
...
fs.writeFileSync('public/js/shared-utils.js', sharedContent);

// Add to index.html
let indexHtml = fs.readFileSync('index.html', 'utf-8');
if (!indexHtml.includes('shared-utils.js')) {
  indexHtml = indexHtml.replace('<script src="/js/auth.js" defer></script>', '<script src="/js/shared-utils.js"></script>\n    <script src="/js/auth.js" defer></script>');
  fs.writeFileSync('index.html', indexHtml);
}

// Helper to remove functions from a file
function removeFunction(filePath, functionNames) {
  let content = fs.readFileSync(filePath, 'utf-8');
  let original = content;

  functionNames.forEach(fn => {
    // Regex to match `function name(args) { ... }` supporting nested brackets somewhat safely for known small functions
    // Alternatively, just a simple regex for the ones we know
    const regex = new RegExp(`(const|let|var)?\\s*${fn}\\s*=\\s*function\\s*\\([^)]*\\)\\s*\\{[\\s\\S]*?\\n\\s*\\};?`, 'g');
    content = content.replace(regex, '');
    const regex2 = new RegExp(`function\\s+${fn}\\s*\\([^)]*\\)\\s*\\{[\\s\\S]*?\\n\\s*\\}`, 'g');
    content = content.replace(regex2, '');
    
    // Replace calls with window.fn
    if (fn !== 'STORAGE_KEY') {
        const callRegex = new RegExp(`(?<!window\\.|function\\s+|const\\s+|let\\s+|var\\s+|\\.)\\b${fn}\\(`, 'g');
        content = content.replace(callRegex, `window.${fn}(`);
    } else {
        const callRegex = new RegExp(`(?<!window\\.|const\\s+|let\\s+|var\\s+|\\.)\\bSTORAGE_KEY\\b`, 'g');
        content = content.replace(callRegex, `window.STORAGE_KEY`);
    }
  });
...
// Files to update
removeFunction('public/js/bookingcart.js', ['STORAGE_KEY', 'money', 'flightPriceAmount', 'readState', 'writeState', 'computeTotals', 'ensureToast', 'toast', 'setText', 'clamp', 'getQuery']);
removeFunction('public/js/stays.js', ['STORAGE_KEY', 'money', 'readState', 'writeState', 'ensureToast', 'toast', 'setText', 'clamp', 'getQuery']);
removeFunction('public/js/deals.js', ['money']);
removeFunction('public/js/visa.js', ['money']);
removeFunction('public/js/visa-tool.js', ['showCountryDetails', 'closeVisaModal', 'getVisaTypeStyle']);
removeFunction('public/js/visa-modal.js', ['showCountryDetails', 'closeVisaModal', 'getVisaTypeStyle']);

// Update PaymentPage.jsx manually
let paymentPage = fs.readFileSync('src/pages/PaymentPage.jsx', 'utf-8');
const fnsToRemove = ['STORAGE_KEY', 'money', 'flightPriceAmount', 'readState', 'writeState', 'computeTotals'];
fnsToRemove.forEach(fn => {
  const regex2 = new RegExp(`function\\s+${fn}\\s*\\([^)]*\\)\\s*\\{[\\s\\S]*?\\n\\s*\\}`, 'g');
  paymentPage = paymentPage.replace(regex2, '');
  const regex3 = new RegExp(`const\\s+${fn}\\s*=\\s*.*?;`, 'g');
  paymentPage = paymentPage.replace(regex3, '');
});

// Fix missing calls
['money', 'flightPriceAmount', 'readState', 'writeState', 'computeTotals'].forEach(fn => {
  const callRegex = new RegExp(`(?<!window\\.|function\\s+|const\\s+|let\\s+|var\\s+|\\.)\\b${fn}\\(`, 'g');
  paymentPage = paymentPage.replace(callRegex, `window.${fn}(`);
});
fs.writeFileSync('src/pages/PaymentPage.jsx', paymentPage);
console.log('Updated src/pages/PaymentPage.jsx');
